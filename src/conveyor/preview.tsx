 ...
 * rack and the pallet use: deciding at render time and then calling
 * `useRegistry` inside the resulting branch is a rules-of-hooks violation, and
 * it throws the moment a node crosses between preview and committed on one
 * component instance.
 *
 ...
 */
export default function ConveyorRollerPreview({ node }: { node: ConveyorRollerNode }) {
  const ref = useRef<Group>(null)
  const geometry = useMemo(() => getConveyorGeometry(node, 'full'), [node])
