...
/**
 * Dragging an M7 bay onto the end of a run.
 *
 ...
 * inside an 8 cm window, grid snap fights a 1.95 m pitch that is a multiple of
...
/** Half a metre — the same figure the other two racking kinds use, so a user
 *  who has learned the gesture on one gets it on all three. */
...
function build(nodes: Readonly<Record<string, unknown>>): SeamIndex {
  ...
  for (const value of Object.values(nodes)) {
    ...
    const bay = value as LongspanNode
 ...
 * builder then refuses to merge. Note what the shape key does **not** include:
 * the level list. Two bays of the same frame carrying entirely different
 * shelves share a ladder, which is the whole point of a longspan run.
 */
