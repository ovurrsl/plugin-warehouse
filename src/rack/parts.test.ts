...
describe('run width follows the beam and upright dimensions', () => {
  test('total width is bays of beam-plus-post, closed by one more post', () => {
    for (const config of [
      { bayCount: 1 },
      { bayCount: 3 },
      { bayCount: 12, bayClearWidth: 3.9, uprightWidth: 0.101 },
    ]) {
      const r = rack(config)
      expect(totalWidth(r)).toBeCloseTo(
        r.bayCount * (r.bayClearWidth + r.uprightWidth) + r.uprightWidth,
        9,
      )
  ...
  test('the bay pitch is one beam plus one post, with no slack', () => {
    const r = rack({ bayCount: 5 })
    const frames = frameCentersX(r)
    for (let index = 1; index < frames.length; index++) {
      expect((frames[index] ?? 0) - (frames[index - 1] ?? 0)).toBeCloseTo(bayPitch(r), 9)
    }
