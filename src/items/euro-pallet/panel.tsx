...
export default function EuroPalletPanel() {
  ...
  const node = useScene((s) =>
    selectedId ? (s.nodes[selectedId as AnyNode['id']] as unknown as EuroPalletNode | undefined) : undefined,
  )
  ...
  const handleDuplicate = useCallback(() => {
    ...
    const proto = {
      ...
      type: 'warehouse:euro-pallet' as const,
      name: node.name || 'EUR-Pallet',
  ...
  if (!node || (node.type as string) !== 'warehouse:euro-pallet' || !selectedId) return null

  const pos = node.position ?? [0, 0, 0]
  const rot = node.rotation ?? [0, 0, 0]

  return (
    <PanelWrapper icon="/icons/shelf.webp" onClose={handleClose} title={node.name || 'EUR-Pallet'} width={300}>
