...
import { getLongspanGeometry } from './geometry-builder'
import { hasRightNeighbour } from './neighbours'
import type { LongspanNode } from './schema'

/**
 * Baked `/viewer` için M7 bay'inin kolektif statik çizicisi — rafın muadili.
 *
 ...
 */
export default makeBakeReplaceRenderer<LongspanNode>((nodes, appearance) => {
  const record: Record<string, unknown> = {}
  for (const node of nodes) record[node.id] = node
  const material = getRackMaterial(appearance)
  return groupByGeometry(
    nodes,
    (node) => getLongspanGeometry(node, 'full', { omitRight: hasRightNeighbour(record, node.id) }),
    () => material,
  )
})
