...
import ToteCartPreview from './preview'
...
/**
 * Toplama arabası yerleştirme aracı.
 *
 * `[` / `]` KAT SAYISINI dolaşıyor — arabanın yerleştirme sırasında
 * gerçekten ayarlanan tek ölçüsü, ve boyu değiştiren şey o (yükseklik
 * katlardan türüyor). Kasa boyu ve tekerlek panelden: ikisi de bir kez
...
export default function ToteCartTool() {
  ...
  const previewNode = useMemo(
    () => ToteCartNode.parse({ ...brush, position: [0, 0, 0], rotation: [0, 0, 0] }),
  ...
  const boxDimensions = useMemo(
    (): [number, number, number] => [
      cartLengthM(previewNode),
      overallHeightM(previewNode),
      cartWidthM(previewNode),
    ],
    [previewNode],
  )
  ...
  useEffect(() => {
    ...
    const recomputeValidity = (position: [number, number, number]) => {
      ...
      const { valid: placeable } = spatialGridManager.canPlaceOnFloor(
        activeLevelId,
        position,
        [cartLengthM(ghost), overallHeightM(ghost), cartWidthM(ghost)],
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === '[' || event.key === ']') {
        ...
        const next = ghostRef.current.tiers + step
        // Uçlarda sarmıyor: sekizden bire atlamak, tuşu basılı tutan
        // kullanıcının arabayı sessizce tek katlıya indirmesi demekti.
        if (next < MIN_TIERS || next > MAX_TIERS) return
        setBrush({ tiers: next })
        return
      }
  ...
  return (
    <>
      <group layers={EDITOR_LAYER} ref={cursorRef} visible={cursorVisible}>
        <ToteCartPreview node={previewNode} />
      </group>
      {cursorVisible && (
        <PlacementBox
