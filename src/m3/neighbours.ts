...
/**
 * Which M3 bays abut which.
 *
 * Bays share frames — N bays stand on N+1 frames. Each bay builds its **left**
 * frame always and its **right** frame only when nothing abuts it, so two bays
 * at one pitch produce three frames and one row of posts at the seam.
 *
 * The fourth copy of this file after the selective rack, the drive-in lane and
 * the M7 bay, and deliberately not shared with any of them: the shape key names
 * the fields *this* kind must agree on before two frames genuinely coincide,
 * and letting four kinds match each other would merge posts of different
 * sections at different depths with different punch patterns.
 *
 ...
 * key becomes a lookup rather than an allocation.
 */
 ...
 *
 * **`frameVariant` is in it**, and that is the difference from the M7 key. An
 * M3 frame comes in five models, three of which fill the frame plane with a
 * sheet or a mesh — so a plain frame and a panelled one at the same place are
 * not the same ladder, and merging them would silently delete a panel the user
 * ordered.
 *
 * The **levels are not** in it. Two bays on one frame can carry entirely
 * different shelves, drawers and dividers, which is the ordinary way a picking
 * run is laid out.
 */
...
export function rightNeighbourPosition(bay: M3ShelvingNode): [number, number] {
...
function build(nodes: Readonly<Record<string, unknown>>): ReadonlySet<string> {
  ...
  for (const { bay, shape } of bays) {
    const [x, z] = rightNeighbourPosition(bay as unknown as M3ShelvingNode)
