    ...
    (node): Issue[] => {
      ...
      if (!carriesShortestBox(node)) {
        issues.push({
          field: 'shortestBox',
          severity: 'warning',
          msg: `A ${(node.shortestBox * 1000).toFixed(0)} mm box spans ${rollersUnderShortestBox(node)} rollers across the widest ${(widestRollerGapM(node) * 1000).toFixed(0)} mm gap; the catalogue asks for ${MIN_ROLLERS_UNDER_A_BOX}. The strips set that spacing, so the box is what has to change.`,
        })
      }
