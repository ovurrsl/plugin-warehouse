...
/**
 * Placement for a booster.
 *
 * One at a time: a booster is a control element rather than a length of line —
 * you put one where the cycle needs regulating, not twenty in a row.
 *
...
export default function ConveyorBoosterTool() {
  ...
  const previewNode = useMemo(
    () => ConveyorBoosterNode.parse({ position: [0, 0, 0], rotation: [0, 0, 0] }),
    [],
  )
  ...
  useEffect(() => {
    ...
    const recomputeValidity = (visual: [number, number, number]) => {
      ...
      const placed = ConveyorBoosterNode.parse({
        ...previewRef.current,
        id: undefined,
        position: visual,
    ...
    const applyCursor = (position: [number, number, number]) => {
      ...
      // A booster's steel is centred on its node, so the measuring box and the
  ...
  return (
    <>
      <group layers={EDITOR_LAYER} ref={cursorRef} visible={cursorVisible}>
        <ConveyorBoosterPreview node={previewNode} />
      </group>
      {cursorVisible && (
        <PlacementBox
